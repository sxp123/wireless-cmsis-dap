...
uint8_t data_transfer_process(void)
{
	...
	if ((USB_RequestOut != USB_RequestIn) || USB_RequestFlag)
	{
		...
		if(n == DAP_PACKET_COUNT)
			n=0;
		USB_RequestOut = n;
		if (USB_RequestOut == USB_RequestIn) //when buffer is fullfilled
			USB_RequestFlag = 0;
		if (USB_ResponseIdle)
...
uint8_t dap_process(void)
{
	static uint8_t transfer_start=0;
	...
	if(UART2_TxStatus()==0)
	{
		if(USB_ResponseOut != USB_ResponseIn)
		{
				UART2_Send(USB_Response[USB_ResponseOut],DAP_PACKET_SIZE);
				n=USB_ResponseOut+1;
				if(n==DAP_PACKET_COUNT)
					n=0;
				USB_ResponseOut=n;	
