...
*/
int32_t USART2_Initialize()
{
uint32_t data_bits = 8U, parity = 0U, stop_bits = 1U;
	int32_t  status;
...
{
    ptrUART->Send(data,len);
}

void UART2_Receive(uint8_t *data,uint32_t len)
{
	ptrUART->Receive(data,len);
}

uint8_t UART2_TxStatus()
{
	return ((uint8_t)ptrUART->GetStatus().tx_busy);
}
...
static void UART_Callback (uint32_t event) {
	...
	if (event & ARM_USART_EVENT_RECEIVE_COMPLETE) {
		...
		{			// Discard packet when buffer is full
				if(uart_rx_buf[0]!=ID_DAP_Free)
				{
						memcpy(USB_Request[USB_RequestIn],uart_rx_buf,UART_BUFFER_SIZE);
						USB_RequestIn++;
		...
		#ifdef MY_DEBUG
		ptrUART->Send(USB_Request[USB_RequestIn-1], UART_BUFFER_SIZE);
		#endif
	}
...
int32_t USBD_CDC_ACM_PortInitialize (void) {
	...
	USART2_Initialize();
	//Delayms(100);
//  ptrUART->Send("welcome!",sizeof("welcome!"));
	return(1);
}
