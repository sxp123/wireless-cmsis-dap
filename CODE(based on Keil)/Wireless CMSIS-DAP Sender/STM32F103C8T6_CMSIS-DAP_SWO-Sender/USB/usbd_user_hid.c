...
uint8_t data_transfer_process(void)
{
	uint32_t n;
	uint8_t i;
	uint8_t free_data=0xfc;

	if(UART2_TxStatus()==0)
	{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  
		if ((USB_RequestIn != USB_RequestOut) || USB_RequestFlag)
		{
				if(USB_Request[USB_RequestOut][0]==0x00)
				{
						if(USB_Request[USB_RequestOut][1]==0x01)
						{
							USB_Response[USB_ResponseIn][0]=0x00;
							USB_Response[USB_ResponseIn][1]=0x00;
							n = USB_ResponseIn + 1;
							if (n == DAP_PACKET_COUNT)
								n = 0;
							USB_ResponseIn = n;
						}
						else if(USB_Request[USB_RequestOut][1]==0x02)
						{
							USB_Response[USB_ResponseIn][0]=0x00;
							USB_Response[USB_ResponseIn][1]=0x00;
							n = USB_ResponseIn + 1;
						...
						else if(USB_Request[USB_RequestOut][1]==0x03)
						{
							USB_Response[USB_ResponseIn][0]=0x00;
							USB_Response[USB_ResponseIn][1]=0x00;
							n = USB_ResponseIn + 1;
						...
						else if(USB_Request[USB_RequestOut][1]==0x04)
						{
							USB_Response[USB_ResponseIn][0]=0x00;
							USB_Response[USB_ResponseIn][1]=0x06;
							USB_Response[USB_ResponseIn][2]=0x32;
							USB_Response[USB_ResponseIn][3]=0x2e;
							USB_Response[USB_ResponseIn][4]=0x30;
							USB_Response[USB_ResponseIn][5]=0x2e;
							USB_Response[USB_ResponseIn][6]=0x30;
							USB_Response[USB_ResponseIn][6]=0x00;
							n = USB_ResponseIn + 1;
							if (n == DAP_PACKET_COUNT)
								n = 0;
							USB_ResponseIn = n;						
						}
						else if(USB_Request[USB_RequestOut][1]==0xf0)
						{
							USB_Response[USB_ResponseIn][0]=0x00;
							USB_Response[USB_ResponseIn][1]=0x01;
							USB_Response[USB_ResponseIn][2]=0x37;
							n = USB_ResponseIn + 1;
							if (n == DAP_PACKET_COUNT)
								n = 0;
							USB_ResponseIn = n;						
						}
						else if(USB_Request[USB_RequestOut][1]==0xfe)
						{
							USB_Response[USB_ResponseIn][0]=0x00;
							USB_Response[USB_ResponseIn][1]=0x01;
							USB_Response[USB_ResponseIn][2]=0x40;
							n = USB_ResponseIn + 1;
							if (n == DAP_PACKET_COUNT)
								n = 0;
							USB_ResponseIn = n;						
						}
						else if(USB_Request[USB_RequestOut][1]==0xff)
						{
							USB_Response[USB_ResponseIn][0]=0x00;
							USB_Response[USB_ResponseIn][1]=0x02;
							USB_Response[USB_ResponseIn][2]=0x40;
							USB_Response[USB_ResponseIn][3]=0x00;
							n = USB_ResponseIn + 1;
							if (n == DAP_PACKET_COUNT)
								n = 0;
							USB_ResponseIn = n;						
						...
						{
								UART2_Send(USB_Request[USB_RequestOut],DAP_PACKET_SIZE);
						}
				}
//				else if(USB_Request[USB_RequestOut][0]==0x18)
//				{
//				
//				}
				else
				...
				if(n == DAP_PACKET_COUNT)
					n=0;
				USB_RequestOut = n;
				if (USB_RequestOut == USB_RequestIn) //when buffer is cleaned
					USB_RequestFlag = 0;
		}	
		else
		{
	...
	if(USB_ResponseIn != USB_ResponseOut)
	{
		...
		{	// Request that data is send back to host???
			USB_ResponseIdle = 0;
			usbd_hid_get_report_trigger(0, USB_Response[USB_ResponseOut], DAP_PACKET_SIZE);
			n = USB_ResponseOut+1;
			if(n == DAP_PACKET_COUNT)
				n=0;
			USB_ResponseOut = n;			
